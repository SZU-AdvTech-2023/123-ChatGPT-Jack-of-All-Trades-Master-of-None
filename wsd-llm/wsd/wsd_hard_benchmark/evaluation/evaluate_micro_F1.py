...
def evaluate_from_path(pred_path, true_path, key_subset_path = None):
    """
    ...
    if not key_subset_path:
        result = evaluate(gold_keys, pred_keys)
    else:
        eval_keys = load_keys(key_subset_path)
        result = evaluate(gold_keys, pred_keys, eval_keys=eval_keys)
    return result
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=os.path.join(common_path.project_dir,
                             'data/wsd-hard-benchmark/evaluation/gold/ALLamended.gold.key.txt')
    )
    parser.add_argument(
        '--pred_path',
        type=str,
        ...
        default=os.path.join(common_path.project_dir,
                             'data/wsd-hard-benchmark/evaluation/predictions/ALLamended/gpt-3.5-turbo-0301-predictions.ALLamended.key.txt')
    )
    parser.add_argument(
        '--key_subset_path',
        type=str,
